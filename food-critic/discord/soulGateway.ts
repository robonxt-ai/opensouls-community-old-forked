...
export class SoulGateway {
  ...
  async handleMessage(discordMessage: Message) {
    ...
    const messageHasImageAttachment = discordMessage.attachments.some((attachment) =>
    ...
    const imageUrl = discordMessage.attachments.find((attachment) => attachment.contentType?.startsWith("image"))?.url;
    if (!imageUrl) {
      return;
    }
