...
Integrating a soul with your application is simple via the `@opensouls/souls` API. The API offers `SoulSession`, a client that manages connections with a soul running on the engine, including across sessions with different users.

```javascript
import { SoulSession, Actions, said, Events } from "soul-engine/soul";

// connect to the hosted soul running the subroutine "opensouls/samantha-provokes"
const samantha = new SoulSession({
  organization: "opensouls",
  subroutine: "samantha-provokes",
})

// start a new session with the soul
const sessionId = await samantha.start()
...
Any session can be resumed at a later point in time as well
...
await samantha.resume(sessionId)
```
...
The `SoulSession` API provides a convenient way to integrate hosted souls defined via the [Engine API](#engine-api) into your applications.
...
*Methods*:
- `start()`: initiates execution of the new soul's [Subroutine](#soul-blueprint), returns the promise of a `sessionId` upon connection
- `resume(sessionId: string)`: *resumes* execution of the new soul's [Subroutine](#soul-blueprint) in the session associated with the `sessionId`, returns a promise of the sessionId upon connection
...
```
const { set, get, search } = useVectorStore("your-bucket-name")
```
