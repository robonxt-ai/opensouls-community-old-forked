...
const assists: MentalProcess = async ({ step: initialStep }) => {
  ...
  const shouldStartPoker = await lastStep.compute(
    mentalQuery(`
  ...
  if (shouldStartPoker) {
    const finalStep = lastStep.withMonologue(html`
      ${initialStep.entityName} thought to themself: I will start a game of poker with the user, and shuffle the virtual
